...
def get_args():
    parser = argparse.ArgumentParser("Deep Q Network with Flappy Bird")
...
def train(args):
    ...
    while iter < args.num_iterations:
        prediction = model(state)[0]
        epsilon = args.final_epsilon + (
            (args.num_iterations - iter) * (args.initial_epsilon - args.final_epsilon) / args.num_iterations
        )
        u = random()
