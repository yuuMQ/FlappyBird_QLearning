...
class FlappyBird(object):
    ...
    fps = 30
    pipe_gap_size = 100
    pipe_velocity_x = -4 # tốc độ ống trôi về bên trái

    # parameters for bird
    min_velocity_y = -8 # tốc độp rơi tối thiểu
    max_velocity_y = 10 # tốc độ rơi tối đa
    downward_speed = 1 # Trong luc
    upward_speed = -9 # Luc khi chim bay
    ...
    def is_collided(self):
        # Check if bird torch ground
        if self.bird_height + self.bird_y + 1 >= self.base_y:
            return True
        bird_bbox = Rect(self.bird_x, self.bird_y, self.bird_width, self.bird_height) # (x, y, width, height)
        pipe_boxes = []
        for pipe in self.pipes:
            pipe_boxes.append(Rect(pipe["x_upper"], pipe["y_upper"], self.pipe_width, self.pipe_height)) # Ong tren
            pipe_boxes.append(Rect(pipe["x_lower"], pipe["y_lower"], self.pipe_width, self.pipe_height)) # Ong duoi

            # Check if the bird's bounding box overlaps to the bounding box of any pipe
            if bird_bbox.collidelist(pipe_boxes) == -1:
                return False

            # Duyet qua ong tren va ong duoi
            for i in range(2):
                ...
                if np.any(
                        self.bird_hitmask[self.bird_index][min_x1:min_x1 + cropped_bbox.width,
    ...
    def next_frame(self, action):
        ...
        if 0 < self.pipes[0]["x_lower"] < 5:
            self.pipes.append(self.generated_pipe())
